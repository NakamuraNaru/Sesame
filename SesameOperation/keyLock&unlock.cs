...
namespace SesameOperation
{
    ...
    public static class KeyStateChange
    {
            ...
            ILogger log)
        {
            ...
            {
                string sesame_info_body = await response.Content.ReadAsStringAsync();   //HTTP通信で得たbody(device id, serial, nickname)を取得する
                //セサミのデバイスIDを取得する
                string info_temp = sesame_info_body.Replace("[", "").Replace("]", "").Replace("{", "").Replace("}", "");   //不必要な文字を削除
                String[] info_array = info_temp.Split(",");    //device id, serial, nicknameの文字列を格納している配列(使うにはsplitが必要)
                device_id = info_array[0].Split(":")[1].Replace("\"", "").Replace(" ", "");  //不必要な空白とダブルクォーテーションを除去したデバイスID
                //全セサミのデバイスIDを格納する
                //int j = 0;
            ...
            //Sesameの現状態を取得する
            request = new HttpRequestMessage(HttpMethod.Get, url + $"/sesame/{device_id}");    //メッセージ(要求クエリ)
            request.Headers.Add("Authorization", api_key);              //ヘッダーを追加
            response = await client.SendAsync(request);             //非同期操作でHTTP要求を送信(awaitは結果待ちの状態を表す。結果を得ると以降の処理が進む。)
            string status_body = await response.Content.ReadAsStringAsync();   //HTTP通信で得たbody(locked, battery, responsive)を取得する
                                                                                 //セサミのデバイスIDを取得する
            string status_temp = status_body.Replace("{", "").Replace("}", "");   //不必要な文字を削除
            String[] status_array = status_temp.Split(",");     //locked, battery, responsiveの文字列を格納している配列(使うにはsplitが必要)
            var locked = status_array[0].Split(":")[1].Replace(" ", "");         //ロック状況
            var battery = status_array[1].Split(":")[1].Replace(" ", "");        //バッテリー
            var responsive = status_array[2].Split(":")[1].Replace(" ", "");     //使用可能か

            //Console.WriteLine(locked);
            //Console.WriteLine(battery);
            //Console.WriteLine(responsive);

            string user_select = req.Query["operation"];    //ユーザがURLに入力した開け閉めを取得する
            Console.WriteLine(locked);
            ...
            if ((locked == "false" && user_select == "lock") || (locked == "true" && user_select == "unlock"))
            {
                ...
                string taskid_body = await response2.Content.ReadAsStringAsync();                     //HTTP通信で得たbody(タスクID)を取得する
